\chapter{Named Entity Recognition: A Brief Introduction}
\label{cha:ner_intro}
...
This partly stems from the problem of \textbf{domain adaptation}: Most of the existing \gls{ner} systems are trained on a small selection of standard corpora mainly consisting of newswire texts. But social media texts are often \textbf{more informal and noisy} than newspaper articles, they contain misspellings or domain-specific expressions such as hashtags. 
...
Earlier studies found that applying a system trained on newswire corpora to social media texts would result in a drop of performance (cf. \cite{ritter_2011}). 
...
\gls{ner} systems differ in terms of how they can (or cannot) take into account the context of a word. Moreover, providing context is also a matter of data: User-generated texts from social media are often of \textbf{shorter length} than e.g. newspaper articles which might result in a lack of context (cf. \cite{ritter_2011}).
