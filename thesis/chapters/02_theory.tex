...
Precision thus represents the percentage of correctly identified named entities out of all recognized named entities, (i.e. the percentage of true positives out of all positive predictions), and recall equates to the percentage of named entities that are recognized by the model out of all tokens tagged as named entities in the test set. 
...
Judging from the F1 scores of the respective winning systems, the task of NER in social media texts continued to be a considerable challenge. Furthermore, the ongoing shortage of manually annotated and sufficiently large corpora for this task becomes apparent. However, the results might also indicate that by then state-of-the-art machine learning systems had reached their limits. The following section traces the evolvement of NER approaches over time and gives insights into the respective potentials and limitations. 
 
\section{Overview of NER approaches}

Since it's beginnings in the 1970s, NER approaches have undergone several phases of development, parallel to the advancements in NLP technologies:  Starting with early rule-based and knowledge-based systems, the field moved on to machine-learning approaches based on feature-engineering. The development of NER systems based on neural networks, relying only on minimal feature-engineering, marked another important step forward in the field. Lately, the introduction of transformer models brought along yet another crucial advancement and achieve new state of the art performance for several NLP tasks including NER. 
...
Machine learning approaches resign from the concept of handcrafted rules and instead focus on the development of algorithms which are able to perform a given task by automatically learning from a large number of examples, the training data (cf. \cite[14]{vajjala_practical_2020}). 
The process usually involves the creation of a numeric representation of the training data. These features are then processed by the model which is supposed to patterns in the training data from them (cf. \cite[15]{vajjala_practical_2020}).

The introduction of machine learning approaches thus allowed for the development of NER models through an automated learning process. 

Machine learning algorithms can roughly be grouped into three paradigms: supervised learning, unsupervised learning, and reinforcement learning. For the sake of brevity, I will focus on supervised learning and not go into further detail about the latter two concepts. 

\subsubsection{Feature-engineered supervised systems}

In supervised learning, the machine learning model learns to make predictions for unseen data from a large number of examples which represent input-output pairs. Regarding NER, an input example could be a text containing different instances of entities. The output would then be the corresponding spans and labels which determine the type of each given entity in the text example. These outputs are also called labels or ground truth. 
In order to yield a well-performing model which is able to predict the spans and labels for entities in unseen texts, large amounts of labeled data are required (cf. \cite[15]{vajjala_practical_2020}).
...
Commonly used models for NER based on machine learning are Hidden Markov Models (HMM), Maximum Entropy Models (MEM), Support Vector Machines (SVM), Decision Trees, and Conditional Random Fields (CRF) (cf.  \cite{yadav_survey_2019}). As two examples, I will briefly describe HMMs and CRF.
...
Hidden Markov Models are based on the concept of an underlying structure, called hidden state which is assumed to generate the observed state of data. Regarding text data, this would be an underlying grammar which is hidden, but actually generates the order of parts of speechs in a text: 

 
 statisical model which is based on the assumption that data is 
...
%- uses seeds to discover text having potential named entities, detects noun phrases and filters any with low IDF values, and feeds the filtered list to a classifier \cite{yadav_survey_2019}
 ...
 Limitations: 
 feature-engineering (aufwändig)
 sparse vectors
 no word embeddings
 what about OOV words?
